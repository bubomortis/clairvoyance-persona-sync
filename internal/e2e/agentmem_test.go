...
func TestRoundtrip_AgentMemory(t *testing.T) {
	...
	srcMemDir := clv.AgentMemoryDir(srcHome, ghostCwd)
	...
	dstMemDir := clv.AgentMemoryDir(dstHome, dstDir)
	...
	if _, err := os.Stat(clv.AgentMemoryDir(dstHome, ghostCwd)); !os.IsNotExist(err) {
...
func TestRoundtrip_AgentMemory_OptInOnly(t *testing.T) {
	...
	write(t, filepath.Join(clv.AgentMemoryDir(srcHome, `Z:\ghost\ws`), "MEMORY.md"), "secret plan\n")
...
func TestExport_AgentMemory_BlocksPlantedSecret(t *testing.T) {
	...
	write(t, filepath.Join(clv.AgentMemoryDir(srcHome, `Z:\ghost\ws`), "leak.md"),
