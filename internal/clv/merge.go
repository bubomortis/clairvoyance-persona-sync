...
func (in *Instance) MergeStaffEntry(profile string, incoming json.RawMessage, id string, mode Mode, dryRun bool) (action string, changed, preserved []string, err error) {
	...
	if dryRun {
		return action, changed, preserved, nil
	}
	...
	return action, changed, preserved, os.WriteFile(p, nb, 0o644)
}
