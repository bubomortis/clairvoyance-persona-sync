...
func printReport(rep *importer.Report) {
	...
	if rep.MemoryPlaced {
		...
		fmt.Println("  ⓘ imported memory takes effect at the affected persona(s)' NEXT SESSION START — fully restart the persona (start a new session) to load it; an already-running persona will not see it until then. The app-closed import + relaunch does this for you.")
	}
	if rep.ReceiptPath != "" {
